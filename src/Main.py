...
class Solver:
    ...
    def __init__(self, start_x, start_y, start_dir, use_modified):
        # Maze size
        self.LENGTH = API.mazeWidth()
    ...
    def visualize_flood_fill_and_visited(self):
        for x in range(self.LENGTH):
            for y in range(self.LENGTH):
                if (self.visited[y][x]):
                    API.setText(x, self.LENGTH - y - 1,
                                self.floodfill_array[y][x])
                    API.setColor(x, self.LENGTH - y - 1, 'Y')
    ...
    def build_fast_run(self):
        ...
        while True:
            ...
            min_flood_val, next_coord = min(
                ((self.floodfill_array[neighbor[1]][neighbor[0]], neighbor) for neighbor in neighbors))
            next_dir = self.get_next_dir(*next_coord)
            API.setColor(next_coord[0], self.LENGTH - next_coord[1] - 1, 'G')
            ...
            match self.fast_run_path[i]:
                case Commands.FORWARD:
                    ...
                    while (i < len(self.fast_run_path) - 1 and self.fast_run_path[i + 1] == Commands.FORWARD):
                        forward_steps += 1
                        i += 1
                    # log(f"Moving forwards {forward_steps} steps")
                    API.moveForward(forward_steps)
...
def main():
    ...
    solver = Solver(START_X, START_Y, START_DIR, USE_MODIFIED_FLOODFILL)
    log("Running")
    API.setColor(0, 0, "G")
    API.setText(0, 0, "abc")
    API.ackReset()  # Allow the mouse to be moved back to starting square

    # Search run
    log("Search run started")
    solver.search_run()
    solver.search_run_back(to_start=False)
    solver.search_run()
    solver.search_run_back(to_start=True)
    log("Fast run started")
    # Fast run
    solver.build_fast_run()
    solver.fast_run()
