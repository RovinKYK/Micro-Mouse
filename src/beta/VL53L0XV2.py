...
class VL53L0X:
    ...
    def __init__(self, i2c, address=0x29, io_timeout_s=0):
        ...
        # self._bus.write_i2c_block_data(_GLOBAL_CONFIG_SPAD_ENABLES_REF_0, 0,  list(ref_spad_map)[0:1])
        ref_spad_map.extend(self._bus.read_i2c_block_data(self.address, _GLOBAL_CONFIG_SPAD_ENABLES_REF_0, 6))
        ref_spad_map = bytearray(ref_spad_map)
        # with self._device:
        ...
        self._bus.write_i2c_block_data(self.address, _GLOBAL_CONFIG_SPAD_ENABLES_REF_0,  list(ref_spad_map[1:]))
        # with self._device:
    ...
    def _read_u8(self, registry):
        # Read an 8-bit unsigned value from the specified 8-bit address.
        return self._bus.read_byte_data(self.address, registry)

    def _read_u16(self, registry):
        # Read a 16-bit BE unsigned value from the specified 8-bit address.
        low = self._bus.read_byte_data(self.address, registry)
        high = self._bus.read_byte_data(self.address, registry + 1)
        val = (high << 8) + low
        return val

    def _write_u8(self, registry, val):
        # Write an 8-bit unsigned value to the specified 8-bit address.
        self._bus.write_byte_data(self.address, registry, val)

    def _write_u16(self, registry, val):
